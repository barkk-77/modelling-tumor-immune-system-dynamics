...
[t_post_7,y_post_7]=ode15s(@simul_diff_hetero_growth,trange1,cond2);
...
[t_post_14,y_post_14]=ode15s(@simul_diff_hetero_growth,trange2,cond3);


t=[t_for_7',(t_post_7'+7),(t_post_14'+14)];
Cp=[y_for_7(:,1)',y_post_7(:,1)',y_post_14(:,1)'];
Cq=[zeros(1,length(y_for_7(:,1)')),y_post_7(:,2)',y_post_14(:,2)'];
N=[y_for_7(:,2)',y_post_7(:,3)',y_post_14(:,3)'];
T=[y_for_7(:,3)',y_post_7(:,4)',y_post_14(:,4)'];
len = length(t);
...
for i=1:len
    ...
    if t(i) >= 7 && t(i) <= 12
    annotation('textarrow', [0.208, 0.208], [0.38, 0.3], 'String', '1st Dose of 2x10^6 quiescent cells');
    end

    if t(i) >= 14 && t(i) <= 25
    annotation('textarrow', [0.285, 0.285], [0.38, 0.3], 'String', '2nd Dose of 2x10^6 quiescent cells');
    end
...
movie = VideoWriter('(7,14 days before inoculation) Simulation of Vaccination Protocol','MPEG-4');
movie.FrameRate = 12;
