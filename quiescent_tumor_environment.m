...
trange = [0 365];
ini = [5e4 2.5e5 5.2e5];
[t, y] = ode15s(@simul_diff_quiescent, trange, ini);
Cq = y(:, 1)';
N = y(:, 2)';
T = y(:, 3)';
...
for i = 1:len
    ...
    Cq_Cubes=grow_cubes([0,0,0],cubes_Cq + randi([-1 1]),[0 1 1]);
    hold on;
    ...
    % Placards and Titles
    legend([Cq_Cubes, N_plot, T_plot], {'500 Quiescent Cells','10x10^3 NK-cells','10x10^3 T-cells'}, ...
    ...
    msg = sprintf(['\\color{white}Quiescent Cells: \\color{cyan}%.1f x10^3\n' ...
                   '\\color{white}NK Cells: \\color{magenta}%.1f x10^6\n' ...
                   '\\color{white}T Cells: \\color{blue}%.1f x10^6'], ...
                   Cq(i)/1e3, N(i)/1e6, T(i)/1e6);
    ...
    annotation('textbox', [0.05, 0.83, 0.1, 0.05], ...
               ...
               'FontWeight', 'normal', ...
               'FontName', 'Helvetica');
    title('Quiescent Tumor Growth', 'Color', 'w', 'FontSize', 16, 'FontWeight', 'bold');
...
movie = VideoWriter('Quiescent Cell Growth Animation','MPEG-4');
movie.FrameRate = 10;
